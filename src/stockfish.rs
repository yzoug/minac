...
use tokio::sync::mpsc;
use vampirc_uci::{parse_with_unknown, UciMessage, MessageList, UciOptionConfig, UciTimeControl, Duration};
...
pub(crate) async fn receive_stockfish_best_move(stockfish_out: ChildStdout, tx: mpsc::Sender<StockfishOutput>) {
        ...
        for uci_message in parse_with_unknown(&next_line) {
            ...
            match uci_message {
                UciMessage::BestMove { best_move, ponder } => {
                    tx.send(StockfishOutput::StockfishBestMove { chess_move: best_move }).await.unwrap();
                }
                _ => ()
            };
        }
...
pub(crate) async fn send_move_to_stockfish(mut stockfish_in: ChildStdin, mut rx: mpsc::Receiver<StockfishInput>) {
    ...
    while let Some(cmd) = rx.recv().await {
        ...
        match cmd {
            StockfishInput::Configure { level, depth } => {
                ...
                messages.push(UciMessage::Option(
                ...
                messages.push(UciMessage::Position { startpos: true, fen: None, moves: Vec::new() });
                messages.push(UciMessage::IsReady);
            },
            StockfishInput::PlayerMove { chess_move, fen } => {
                debug!("Received UCI move from player {}{}, sending it to stockfish", chess_move.get_source(), chess_move.get_dest());
                ...
                messages.push(UciMessage::Go { time_control: Some(time_control), search_control: None });
            },
        }
